...
local function applyPreviewPose(ped)
    if not ped or ped == 0 then return end
    local dict = (Config.PreviewPose and Config.PreviewPose.dict) or 'anim@heists@heist_corona@single_team'
    local anim = (Config.PreviewPose and Config.PreviewPose.anim) or 'single_team_loop_boss'
    local flag = (Config.PreviewPose and Config.PreviewPose.flag) or 1
...
local function initializePedModel(model, data)
    CreateThread(function()
        ...
        SetPedCanRagdoll(charPed, false)
        if data then
            TriggerEvent('qb-clothing:client:loadPlayerClothing', data, charPed)
            Wait(150)
        end
        applyPreviewPose(charPed)
    end)
end
...
local function skyCam(bool)
    ...
    if bool then
        ...
        cam = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', Config.CamCoords.x, Config.CamCoords.y, Config.CamCoords.z, 0.0, 0.0, Config.CamCoords.w, 50.00, false, 0)
        PointCamAtCoord(cam, Config.PedCoords.x, Config.PedCoords.y, Config.PedCoords.z + 0.55)
...
local function openCharMenu(bool)
    QBCore.Functions.TriggerCallback('qb-multicharacter:server:GetNumberOfCharacters', function(result, countries)
        ...
        SendNUIMessage({
            ...
            toggle = bool,
            nChar = result,
            playerName = GetPlayerName(PlayerId()) or 'Oyuncu',
...
RegisterNetEvent('qb-multicharacter:client:spawnAtDefault', function()
    if charPed and DoesEntityExist(charPed) then
        DeletePed(charPed)
        charPed = nil
    end
